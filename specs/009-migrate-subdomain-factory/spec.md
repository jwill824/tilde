...
- **FR-004**: All install script URLs in `site/tilde/index.html` MUST reference `tilde.thingstead.io/install.sh`.
...
- **FR-006**: The `repos.json` entry MUST include `speckit_enabled: true`, `copilot_enabled: true`, and `git_hooks` with lefthook pre-commit and commit-msg hooks — matching the existing thingstead entry pattern.
...
- The factory's `cloudflare.tf` automatically provisions `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` GitHub Actions secrets for any repo with a `cloudflare_pages_projects` entry.
